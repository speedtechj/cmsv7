                            ...
                            ->schema([
                                ...
                                Forms\Components\TextInput::make('invoice')
                                    ...
                                    ->afterStateUpdated(function (Booking $booking, Set $set, Get $get, $state) {
                                        $bookinginfo = Booking::where('booking_invoice', $state)
                                            ->orWhere('manual_invoice', $state)
                                            ->first();
                                  ...
                                  //      dd($state);
                                        if (!$bookinginfo) {
                                            ...
                                            if($state){
                                                 $set('booking_id', $bookinginfo->id);
                                            $set('manual_invoice', $bookinginfo->manual_invoice);
                                            $set('sender', $bookinginfo->sender->full_name ?? '');
                                            $set('receiver', $bookinginfo->receiver->full_name ?? '');
                                            $set('senderaddress', $bookinginfo->senderaddress->address ?? '');
                                            $set('receiveraddress', $bookinginfo->receiveraddress->address ?? '');
                                            }
                                            // $set('booking_id', $bookinginfo->id);
                                            // $set('manual_invoice', $bookinginfo->manual_invoice);
                                            // $set('sender', $bookinginfo->sender->full_name ?? '');
